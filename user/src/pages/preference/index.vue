<template>
  <view class="pref-page">
    <!-- 跳过按钮 -->
    <text class="skip-btn" @tap="onSkip">跳过</text>
    ...
    <view v-if="step == 1" class="pref-body">
      <view class="tag-grid">
        <view
          v-for="item in nationalities" :key="item.key"
          class="tag-item" :class="{ selected: nationality == item.key }"
          @tap="nationality = item.key"
        >
          <AppIcon :name="item.icon as any" :size="64" />
    ...
    <view v-if="step == 2" class="pref-body">
      <view class="tag-grid">
        <view
          v-for="item in intensities" :key="item.key"
          class="tag-item" :class="{ selected: intensity == item.key }"
    ...
    <view v-if="step == 3" class="pref-body">
      <view class="tag-grid">
        <view
          v-for="item in interestOptions" :key="item.key"
          class="tag-item" :class="{ selected: selectedInterests.includes(item.key) }"
...
const stepLabels = ['您来自哪里？', '您偏好哪种活动强度？', '您对哪些类型感兴趣？（可多选）']
...
function applyLocale(nat: string) {
  // 只支持中文和英文，日韩等外国用户默认英文
  const lang = (nat === 'cn_local' || nat === 'cn_other') ? 'zh-CN' : 'en'
...
.step-counter { font-size: 24rpx; color: #bbb; }
...
.tag-text { font-size: 26rpx; color: #333; text-align: center; margin-top: 8rpx; }

.tag-item.selected .tag-text { color: #1890FF; font-weight: 500; }

.tag-desc { font-size: 22rpx; color: #999; margin-top: 6rpx; text-align: center; }
